...
class Mailer extends MailerBase
{
    ...
    /**
     * Helper for send() method, the first argument can take a single mail or an
     ...
     *  - (object) ['mail' => 'admin@domain.tld', 'name' => 'Adam Person']
     *  - (array) ['admin@domain.tld' => 'Adam Person', ...]
     *  - (array) [ (object|array) ['mail' => 'admin@domain.tld', 'name' => 'Adam Person'], [...] ]
        ...
        elseif (is_array($recipients) || $recipients instanceof Collection) {
            foreach ($recipients as $address => $person) {
                ...
                elseif (is_array($person)) {
                    if (!$address = array_get($person, 'mail', array_get($person, 'address'))) {
                        continue;
                    }
